...
public class AspectRatioController : MonoBehaviour
{
    ...
    [SerializeField]
    private bool forceMinimumResolution = false;
    [SerializeField]
    private bool forceMaximumResolution = false;
    ...
    void Start()
    {
        ...
        var aspectRatioControllers = FindObjectsByType<AspectRatioController>(
            FindObjectsInactive.Include, 
            FindObjectsSortMode.InstanceID
        );

        if (aspectRatioControllers.Length > 1)
        {
            DestroyImmediate(this);
            return;
        }

        // Make sure this script's parent isn't destroyed on load, ensuring it'll work correctly
    ...
    IntPtr wndProc(IntPtr hWnd, uint msg, IntPtr wParam, IntPtr lParam)
    {
        ...
        if (msg == WM_SIZING)
        {
            ...
            int newWidth = Mathf.Clamp(
                rc.Right - rc.Left,
                forceMinimumResolution ? minWidthPixel : 1,
                forceMaximumResolution ? maxWidthPixel : 1
            );

            int newHeight = Mathf.Clamp(
                rc.Bottom - rc.Top,
                forceMinimumResolution ? minHeightPixel : 1,
                forceMaximumResolution ? maxHeightPixel : 1
            );
